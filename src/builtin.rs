...
async fn process_exit<E: AsyncWriteExt + Unpin>(
    ...
    if let Ok(hist_file) = env::var("HISTFILE") {
        // TODO: Remove this `state.add_history` statement after codecrafts is no longer a thing, for some reason on that end `exit`
    ...
    match DeclareArgs::try_parse_from(
        ...
        Ok(args) => {
            if let Some(print) = args.print {
                if let Some(var) = state.get_variable(&print) {
                    let _ = stdout
                        .write_all(format!("declare -- {}=\"{}\"\n", print, var).as_bytes())
                        .await;
                } else {
                    let _ = stderr
            ...
            } else if let Some(store) = args.declare.first() {
                let contains_whitspaces = store
                    .chars()
                    .map(|c| c.is_whitespace())
                    .find(|b| *b == true)
                    .unwrap_or(false);
                let split: Vec<&str> = store.split("=").collect();

                if split.len() != 2 || contains_whitspaces {
